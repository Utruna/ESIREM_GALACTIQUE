...
function upgradeBouclier($idJoueur, $idPlanete)
{
    $pdo = new PDO('mysql:host=localhost;dbname=galactique2', 'root', '');
    echo "Début de la vérification";
    $query = "SELECT niveauLabo, niveauTechBouclier FROM infrastructure WHERE idPlanete = :idPlanete";
    $stmt = $pdo->prepare($query);
    ...
    $niveauTechBouclier = $infrastructure['niveauTechBouclier'];

    // Si le niveau max est atteint on n'améliore pas
    if ($niveauTechBouclier >= 10) {
        $_SESSION['bad_alert'] = "Vous avez atteint le niveau maximum pour la technologie Bouclier.";
        header('Location: ./recherche.php');
        exit();
    }
    // Verifier si le niveau de recherche energetique est suffisant
    $query = "SELECT niveauTechIons, niveauTechEnergie FROM infrastructure
    ...
    if ($verif['niveauTechIons'] >= 2 && $verif['niveauTechEnergie'] >= 8) {
        ...
        if (checkRessources($pdo, $idJoueur, $coutMetal, $coutEnergie, $coutDeuterium)) {
            // Si oui, on effectue la recherche pour le Bouclier
            // Mise à jour du niveau de la technologie Bouclier dans l'infrastructure
            $query = "UPDATE infrastructure SET niveauTechBouclier = niveauTechBouclier + 1 WHERE idPlanete = :idPlanete";
            $stmt = $pdo->prepare($query);
            $stmt->bindValue(':idPlanete', $idPlanete);
            $stmt->execute();
            ...
            $_SESSION['good_alert'] = "La recherche pour le Bouclier a bien été effectuée.";
            header('Location: ./recherche.php');
        } else {
        ...
        $_SESSION['bad_alert'] = "Le joueur ne possède pas toutes les recherches nécessaires pour obtenir le Bouclier.";
        header('Location: ./recherche.php');
    }
}
