...
function upgradeArmement($idJoueur, $idPlanete)
{
    $pdo = new PDO('mysql:host=localhost;dbname=galactique2', 'root', '');
    echo "Début de la vérification";
    $query = "SELECT niveauLabo, niveauTechArmement FROM infrastructure WHERE idPlanete = :idPlanete";
    $stmt = $pdo->prepare($query);
    ...
    $niveauTechArmement = $infrastructure['niveauTechArmement'];

    // Si le niveau max est atteint on n'améliore pas
    if ($niveauTechArmement >= 10) {
        $_SESSION['bad_alert'] = "Vous avez atteint le niveau maximum pour la technologie Armement.";
        header('Location: ./recherche.php');
        exit();
    }
    // Verifier si le niveau de recherche energetique est suffisant
    $query = "SELECT niveauLabo FROM infrastructure
                WHERE idPlanete = :idPlanete";
    $stmt = $pdo->prepare($query);
    $stmt->bindValue(':idPlanete', $idPlanete);
    $stmt->execute();
    $verif = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($verif['niveauLabo'] >= 1) {
        $query = "SELECT coutMetal, coutEnergie, coutDeuterium FROM cout WHERE structureType = 'recherche_Armement'";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        ...
        if (checkRessources($pdo, $idJoueur, $coutMetal, $coutEnergie, $coutDeuterium)) {
            // Si oui, on effectue la recherche pour le Armement
            // Mise à jour du niveau de la technologie Armement dans l'infrastructure
            $query = "UPDATE infrastructure SET niveauTechArmement = niveauTechArmement + 1 WHERE idPlanete = :idPlanete";
            $stmt = $pdo->prepare($query);
            $stmt->bindValue(':idPlanete', $idPlanete);
            $stmt->execute();
            ...
            $_SESSION['good_alert'] = "La recherche pour le Armement a bien été effectuée.";
            header('Location: ./recherche.php');
        } else {
        ...
        $_SESSION['bad_alert'] = "Le joueur ne possède pas toutes les recherches nécessaires pour obtenir le Armement.";
        header('Location: ./recherche.php');
    }
}
