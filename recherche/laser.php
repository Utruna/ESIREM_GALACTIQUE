...
function upgradeLaser($idJoueur, $idPlanete)
{
    ...
    if ($niveauTechLaser >= 10) {
        $_SESSION['bad_alert'] = "Vous avez atteint le niveau maximum pour la technologie laser.";
        header('Location: ./recherche.php');
        exit();
    }
    // Verifier si le niveau de recherche energetique est suffisant
    $query = "SELECT niveauTechEnergie FROM infrastructure
                WHERE idPlanete = :idPlanete";
    $stmt = $pdo->prepare($query);
    $stmt->bindValue(':idPlanete', $idPlanete);
    $stmt->execute();
    $verif = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($verif['niveauTechEnergie'] >= 5) {
