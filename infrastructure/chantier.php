...
function upgradeChantier($idJoueur, $idPlanete){
    $pdo = new PDO('mysql:host=localhost;dbname=galactique2', 'root', '');
    ...
    if ($niveauchantier['niveauChantierSpatial'] >= 10) {
        $_SESSION['bad_alert'] = "Vous avez atteint le niveau maximum pour ce batiment !";
        header('Location: ./infrastructure.php');
        exit();
    }
    $query = "SELECT coutMetal, coutEnergie, coutDeuterium FROM cout WHERE structureType = 'chantier_spatial'";
    $stmt = $pdo->prepare($query);
    $stmt->execute();
    ...
    if (checkRessources($pdo, $idJoueur, $coutMetal, $coutEnergie, $coutDeuterium)) {
        ...
        $query = "UPDATE infrastructure SET niveauChantierSpatial = niveauChantierSpatial + 1 WHERE idPlanete = :idPlanete";
        $stmt = $pdo->prepare($query);
        $stmt->bindValue(':idPlanete', $idPlanete);
        $stmt->execute();
        // Mise à jour des ressources du joueur après
        updateRestante($pdo, $idJoueur, $coutMetal, $coutEnergie, $coutDeuterium);
        $_SESSION['good_alert'] = "Le chantier spatial a été amélioré !";
        header('Location: ./infrastructure.php');
    } else {
