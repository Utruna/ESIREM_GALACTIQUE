...
function upgradeSyntheDeut($idJoueur, $idPlanete){
    $pdo = new PDO('mysql:host=localhost;dbname=galactique2', 'root', '');
    $query = "SELECT niveauSynthetiseurDeut FROM infrastructure WHERE idPlanete = :idPlanete";
    $stmt = $pdo->prepare($query);
    $stmt->bindValue(':idPlanete', $idPlanete);
    $stmt->execute();
    $niveauMetal = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($niveauMetal['niveauSynthetiseurDeut'] >= 10) {
    ...
    if (checkRessources($pdo, $idJoueur, $coutMetal, $coutEnergie, $coutDeuterium)) {
        ...
        $query = "UPDATE infrastructure SET niveauSynthetiseurDeut = niveauSynthetiseurDeut + 1 WHERE idPlanete = :idPlanete";
        $stmt = $pdo->prepare($query);
        $stmt->bindValue(':idPlanete', $idPlanete);
        $stmt->execute();
        // Mise à jour des ressources du joueur après
        updateRestante($pdo, $idJoueur, $coutMetal, $coutEnergie, $coutDeuterium);
        $_SESSION['good_alert'] = "Le synthétiseur de deuterium a été améliorer.";
        header('Location: ./infrastructure.php');
    } else {
