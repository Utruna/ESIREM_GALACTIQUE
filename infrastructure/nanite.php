...
function upgradeNanite($idJoueur, $idPlanete){
    $pdo = new PDO('mysql:host=localhost;dbname=galactique2', 'root', '');
    ...
    $niveauLaser = $infrastructure['niveauTechLaser'];
    if ($niveauLaser < 5) {
        $_SESSION['bad_alert'] = "Vous devez avoir un niveau de technologie laser de 5 minimum pour pouvoir construire une usine de nanite !";
        header('Location: ./infrastructure.php');
        exit();
    }
    $query = "SELECT niveauUsineNanite FROM infrastructure WHERE idPlanete = :idPlanete";
    $stmt = $pdo->prepare($query);
    $stmt->bindValue(':idPlanete', $idPlanete);
    $stmt->execute();
    $niveauNanite = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($niveauNanite['niveauUsineNanite'] >= 10) {
    ...
    if (checkRessources($pdo, $idJoueur, $coutMetal, $coutEnergie, $coutDeuterium)) {
        ...
        $query = "UPDATE infrastructure SET niveauUsineNanite = niveauUsineNanite + 1 WHERE idPlanete = :idPlanete";
        $stmt = $pdo->prepare($query);
        $stmt->bindValue(':idPlanete', $idPlanete);
        $stmt->execute();
        // Mise à jour des ressources du joueur après
        updateRestante($pdo, $idJoueur, $coutMetal, $coutEnergie, $coutDeuterium);
        $_SESSION['good_alert'] = "L'usine de nanite a été améliorer.";
        header('Location: ./infrastructure.php');
    } else {
